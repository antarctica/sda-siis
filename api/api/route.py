...
def convert():
    ...
    if (
        source_format == "application/geo+json"
        and destination_format == "application/rtz"
    ):
        return Response(
            _convert_geojson_to_rtz(geojson=request.json), mimetype="text/rtz"
        )
    elif (
        source_format == "application/geo+json"
        and destination_format == "application/rtzp"
    ):
        with TemporaryDirectory() as tmp_dir:
            ...
            return make_response(
                send_file(
                    rtzp_path, attachment_filename="siis-route.zip", as_attachment=True
                )
            )
    elif (
        source_format == "application/rtz"
        and destination_format == "application/geo+json"
    ):
        return _convert_rtz_to_geojson(record_str=request.data.decode())
    elif (
        source_format == "application/rtz" and destination_format == "application/rtzp"
    ):
        with TemporaryDirectory() as tmp_dir:
            rtzp_path = Path(tmp_dir).joinpath("siis-route.rtzp")
        ...
        source_format == "application/rtzp"
        and destination_format == "application/geo+json"
    ):
        with TemporaryDirectory() as tmp_dir:
            rtzp_path = Path(tmp_dir).joinpath("siis-route.rtzp")
        ...
        source_format == "application/rtzp" and destination_format == "application/rtz"
    ):
        with TemporaryDirectory() as tmp_dir:
            rtzp_path = Path(tmp_dir).joinpath("siis-route.rtzp")
            with open(str(rtzp_path), mode="wb") as rtzp_file:
...
def _convert_geojson_to_rtz_config(geojson: dict) -> dict:
    ...
    for index, coordinate in enumerate(geojson["geometry"]["coordinates"]):
        rtz_config["waypoints"].append(
            {
                "id": index,
                "revision": 0,
                "position": {"lat": coordinate[1], "lon": coordinate[0]},
            }
        )
    return rtz_config
...
def _convert_rtz_config_geojson(config: dict) -> dict:
    feature = {
        ...
        "properties": {"route_name": config["route_name"]},
        "id": str(uuid4()),
    }
...
def _convert_geojson_to_rtz(geojson: dict) -> str:
    configuration = MetadataRecordConfigV1(
        **_convert_geojson_to_rtz_config(geojson=geojson)
    )
