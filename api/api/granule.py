...
def build_date_range_query(start_date: datetime, end_date: datetime, limit: Optional[int] = None):
...
def read_all(limit=None, maxage=None, date=None, date_range=None):
    ...
    if date_range:
        try:
            ...
            end = parse_date(end_date, 'date_range') + timedelta(days=1) - timedelta(seconds=1)
        ...
        query = Granule.query.filter(
            Granule.timestamp > aged_timestamp
        ).order_by(desc(Granule.timestamp))
