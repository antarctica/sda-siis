    ...
    <Flex direction="row" gap="2" align="center">
      ...
      <Box flexGrow={1}>
        <Title bold as="h3" size="body" margin={false}>
          {routeGraphic.attributes.route_name}{' '}
        </Title>
        ...
        <Box flexGrow={1}>
          <Flex gap="2" align="center">
            <TextField
              ...
              label="Route name"
              className={css({ mb: '0' })}
              defaultValue={routeGraphic.attributes.route_name}
              errorMessage="Route name must contain only letters, numbers, and spaces"
            />
          </Flex>
          <Text className={css({ textStyle: 'description' })}>
            Total length: {`${length.toFixed(2)} ${getDisplayUnit(defaultMeasurementUnit)}`}
...
export function RouteDetails({
  ...
  const handleSave = React.useCallback(
    (routeName: string) => {
      ...
      newRouteGraphic.attributes.route_name = routeName;
